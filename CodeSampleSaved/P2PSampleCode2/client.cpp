...
void ConnectToServer(int sock, char *username, char *serverip)
{
	...
	sendto(sock, (const char*) &sendbuf, sizeof(sendbuf), 0,
	...
	for (;;)
	{
		...
		if (n > 0)
		{
			...
			{
				int iread = recvfrom( sock, (char *) &usercount, sizeof(int), 0,
						      (sockaddr *) &remote,(socklen_t*) &fromlen);
				if (iread <= 0)
				{
				  return;
				}
	...
	for (int i = 0; i < usercount; i++)
	{
		...
		recvfrom(sock, (char*) node, sizeof(stUserListNode), 0,
			 (sockaddr *) &remote,(socklen_t*) &fromlen);
...
bool SendMessageTo(char *userName, char *Message)
{
	...
	for (int i = 0; i < MAXRETRY; i++)
	{
		...
		int isend = sendto(PrimaryUDP, (const char *) &MessageHead,
				sizeof(MessageHead), 0, (const sockaddr*) &remote,
				sizeof(remote));
		isend = sendto(PrimaryUDP, (const char *) &realmessage,
				MessageHead.iStringLen, 0, (const sockaddr*) &remote,
				sizeof(remote));
...
void ParseCommand(char * CommandLine)
{
	...
	else if (strcmp(Command, "getu") == 0)
	{
		...
		sendto(PrimaryUDP, (const char*) &command, sizeof(command), 0,
				(const sockaddr *) &server, sizeof(server));
	}
}
...
void*  RecvThreadProc( void* lpParameter)
{
	...
	for (;;)
	{
		int iread = recvfrom(PrimaryUDP, (char *) &recvbuf, sizeof(recvbuf), 0,
				(sockaddr *) &remote,(socklen_t*) &sinlen);
		if (iread <= 0)
		{
			printf("recv error\n");
			continue;
		...
		case P2PMESSAGE:
		{
			...
			int iread1 = recvfrom(PrimaryUDP, comemessage, 256, 0,
					(sockaddr *) &remote,(socklen_t*) &sinlen);
		...
		case GETALLUSER:
		{
			int usercount;
			int fromlen = sizeof(remote);
			int iread = recvfrom(PrimaryUDP, (char *) &usercount, sizeof(int),
					     0, (sockaddr *) &remote,(socklen_t*) &fromlen);
			if (iread <= 0)
			{
