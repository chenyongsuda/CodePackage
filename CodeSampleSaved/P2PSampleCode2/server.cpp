...
int main(int argc, char* argv[])
{
	try
	{
		...
		for (;;)
		{
			...
			int ret = recvfrom(PrimaryUDP, (char *) &recvbuf, sizeof(stMessage),
					   0, (sockaddr *) &sender, (socklen_t* )&dwSender);
			if (ret <= 0)
			{
				printf("recv error");
				continue;
				...
				case LOGIN:
					{
							...
							UserIterator != ClientList.end(); ++UserIterator)
						{
							if (strcmp(((*UserIterator)->userName),
								recvbuf.message.loginmember.userName) == 0)
							{
								bFound = true;
								break;
							}
				...
				case GETALLUSER:
					{
						...
						sendto(PrimaryUDP, (const char*) &command, sizeof(int), 0,
							(const sockaddr*) &sender, sizeof(sender));
